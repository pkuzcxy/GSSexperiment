...
public:
    ...
    int edgeQuery(const unsigned char* v1,const unsigned char* v2,int len1,int len2);
    int nodeValueQuery(const unsigned char*v1, int len, int type);
...
int TCM::edgeQuery(const unsigned char* v1,const unsigned char* v2,int len1,int len2)
{
    int min=INT32_MAX;
    for(int i=0;i<hashnum;i++)
    {
...
int TCM::nodeValueQuery(const unsigned char* v1, int len, int type )
{
	if(type==0)
	{
		//int min=65523*1024;
